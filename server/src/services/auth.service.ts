...
const authService = {
  login: async (inputs: LoginUserInputs) => {
    try {
      const userData = await prisma.commonUser.findUnique({
        where: { email: inputs.email },
      });
      if (!userData)
        throw AuthErrorHandler.notFound(
          "Não existe nenhum usuario com esse email",
        );

      const checkPassword = await bc.compare(
        inputs.password,
        userData.password,
      );
      if (!checkPassword)
        throw AuthErrorHandler.unauthorized("Senha incorreta");

      return tokenConfig.generateToken({
      ...
      throw AuthErrorHandler.internal(
        "Erro interno ao tentar realizar o login",
      );
    }
  },
};
