...
const commonUserController = {
  getPayload: async (req: CustomRequest, res: Response) => {
    res.json(req.userLogged);
  },

  getMe: async (req: CustomRequest, res: Response) => {
    const data = await commonUserService.findUser({
      id_user: req.userLogged?.id_user,
    });
    responseOk(res, "Ok", data);
  },
  ...
  put: async (req: CustomRequest, res: Response, next: NextFunction) => {
    try {
      const upCm = await commonUserService.putUserForUniqueKey(
        req.body,
  ...
  addFriend: async (req: CustomRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const initRelation = await commonUserService.addFriend(
  ...
  getMutualFriends: async (
    ...
    try {
      if (!req.userLogged?.id_user) throw UserErrorHandler.unauthorized();
      const mutualRelations = await commonUserService.getMutualFriends(
        req.userLogged.id_user,
      );
  ...
  getFriendRequestsForUser: async (
    req: CustomRequest,
    res: Response,
    next: NextFunction,
  ) => {
    try {
      if (!req.userLogged?.id_user) throw UserErrorHandler.unauthorized();
      const requests = await commonUserService.getFriendRequestForUser(
        req.userLogged.id_user,
      );
  ...
  sendFriendRequest: async (
    ...
    try {
      if (!req.userLogged?.id_user) throw UserErrorHandler.unauthorized();
      const send = await commonUserService.sendRequestFriend(
        req.userLogged?.id_user,
        req.body.id_user,
      );
  ...
  acceptFriendRequest: async (
    ...
    try {
      if (!req.userLogged?.id_user) throw UserErrorHandler.unauthorized();
      const send = await commonUserService.acceptRequestFriend(
        req.body.id_request,
      );
  ...
  denyFriendRequest: async (
    ...
    try {
      if (!req.userLogged?.id_user) throw UserErrorHandler.unauthorized();
      const send = await commonUserService.denyRequestFriend(
        req.body.id_request,
      );
      responseOk(res, "Pedido recusado com sucesso", send);
    } catch (e) {
      next(e);
    }
  },
  ...
  getFriends: async (req: CustomRequest, res: Response, next: NextFunction) => {
    try {
      if (!req.userLogged?.id_user) throw UserErrorHandler.unauthorized();
      const friends = await commonUserService.getFriends(
        req.userLogged.id_user,
        {
    ...
    try {
      ...
      if (!id_user && !email && !cpf) {
        responseOk(
          ...
          (
            await commonUserService.findUsers({
              page: Number(req.query.page ?? 1),
    ...
    try {
      ...
      if (!id_user && !email && !cpf) {
        throw UserErrorHandler.validation(
          "Informe id_user, email ou cpf para buscar o usuário.",
        );
      }
  ...
  get: async (req: CustomRequest, res: Response, next: NextFunction) => {
    try {
      const users = await commonUserService.findUsers(
        {
          page: Number(req.query.page ?? 1),
          limit: Number(req.query.limit ?? 10),
        },

        // { region: String(req.query.region) ?? "RS" },
      );
      responseOk(
        res,
        "consulta feita com sucesso",
        {
          users: users.users.filter(
            (u) => u.id_user !== req.userLogged?.id_user,
          ),
          page: users.page,
          pages: users.pages,
          total: users.total,
